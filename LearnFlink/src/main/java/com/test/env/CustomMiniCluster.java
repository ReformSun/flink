...
public class CustomMiniCluster implements JobExecutorService, AutoCloseableAsync {
	private static final Logger LOG = LoggerFactory.getLogger(CustomMiniCluster.class);
	...
	private final CustomMiniCluster.TerminatingFatalErrorHandlerFactory taskManagerTerminatingFatalErrorHandlerFactory = new CustomMiniCluster.TerminatingFatalErrorHandlerFactory();
	...
	@GuardedBy("lock")
	private CustomMiniCluster.RpcServiceFactory taskManagerRpcServiceFactory;
	 ...
	 */
	public CustomMiniCluster(MiniClusterConfiguration miniClusterConfiguration) {
	...
	public void start() throws Exception {
		synchronized (lock) {
			...
			try {
				...
				final CustomMiniCluster.RpcServiceFactory dispatcherResourceManagreComponentRpcServiceFactory;

				if (useSingleRpcService) {
					// we always need the 'commonRpcService' for auxiliary calls
					commonRpcService = createRpcService(akkaRpcServiceConfig, false, null);
					final CustomMiniCluster.CommonRpcServiceFactory commonRpcServiceFactory = new CustomMiniCluster.CommonRpcServiceFactory(commonRpcService);
					...
					dispatcherResourceManagreComponentRpcServiceFactory = new CustomMiniCluster.DedicatedRpcServiceFactory(akkaRpcServiceConfig, jobManagerBindAddress);
					taskManagerRpcServiceFactory = new CustomMiniCluster.DedicatedRpcServiceFactory(akkaRpcServiceConfig, taskManagerBindAddress);
				}
				...
				dispatcherResourceManagerComponents.addAll(createDispatcherResourceManagerComponents(
					...
					new CustomMiniCluster.ShutDownFatalErrorHandler()
				));
	...
	protected Collection<? extends DispatcherResourceManagerComponent<?>> createDispatcherResourceManagerComponents(
		...
		CustomMiniCluster.RpcServiceFactory rpcServiceFactory,
	 ...
	 */
	protected static class CommonRpcServiceFactory implements CustomMiniCluster.RpcServiceFactory {
	 ...
	 */
	protected class DedicatedRpcServiceFactory implements CustomMiniCluster.RpcServiceFactory {
		...
		@Override
		public RpcService createRpcService() {
			final RpcService rpcService = CustomMiniCluster.this.createRpcService(akkaRpcServiceConfig, true, jobManagerBindAddress);
	...
	private class TerminatingFatalErrorHandlerFactory {
		...
		/**
		 * Create a new {@link MiniCluster.TerminatingFatalErrorHandler} for the {@link TaskExecutor} with
		 * the given index.
		 *
		 * @param index into the {@link #taskManagers} collection to identify the correct {@link TaskExecutor}.
		 * @return {@link MiniCluster.TerminatingFatalErrorHandler} for the given index
		 */
		@GuardedBy("lock")
		private CustomMiniCluster.TerminatingFatalErrorHandler create(int index) {
			return new CustomMiniCluster.TerminatingFatalErrorHandler(index);
		}
	}
}
