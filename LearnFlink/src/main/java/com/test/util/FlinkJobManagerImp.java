...
//    private final String baseUrl = "http://10.4.251.99:8081";
    ...
    public ReadResult runJob(String jobName, String entry_class, String taskid, int status) throws UnsupportedEncodingException {
		return runJob_1_7_1(jobName,entry_class,taskid,status);
    }
	...
	private ReadResult runJob_1_7_1(String jobName, String entry_class, String taskid, int status){
		if (jobName != null){
			Matcher matcher = pattern.matcher(jobName);
			if (matcher.find() && entry_class != null){
				String filename = matcher.group();
				...
				stringBuilder.append("entry-class=").append(entry_class);
//				stringBuilder.append("&savepointPath=").append("/Users/apple/Desktop/state/savepointData/savepoint-89a61d-db9f0be3e421");
				u = u + "?" + stringBuilder.toString();
	...
	private ReadResult runJob_1_7_2(String jobName, String entry_class, String taskid, int status){
		if (jobName != null){
			...
			if (matcher.find() && entry_class != null){
				...
				if (status == 0){
					...
					map.put("savepointPath","/Users/apple/Desktop/state/savepointData");
				}
